...
if __name__ == "__main__":
    ...
    with open(input_lex,'r') as lex:
        ...
        for line in lex:
            ...
            if lexSplit[1] in catWord:
                catWord[lexSplit[1]].append(lexSplit[0])
            else:
                tmp = []
                tmp.append(lexSplit[0])
                catWord[lexSplit[1]] = tmp
    ...
    with open(input_morph,'r') as morph:
        ...
        for line in morph:
            ...
            if i == 0:
                ...
                if tuple[1] not in catList:
                    catList.append(tuple[1])
                n = tuple[3]
            i += 1
    ...
    for word in wordList:
        ...
        for i in range(0, len(word)):
            if i == 0:
                ...
                if len(word) == 1:
                    fsa.write('(w0 (w'+str(state)+' \"'+str(word[i])+'\" \"'+word+" "+wordCat[word]+'\"))\n')
                    break
            else:
                if i == len(word)-1:
                    fsa.write('(w'+str(state+j)+' (w'+str(state+j+1)+' \"'+str(word[i])+'\" \"'+word+" "+wordCat[word]+'\"))\n')
                else:
            ...
            if (s,key) in transitions:
                for t in transitions[(s,key)]:
                    if key == '*e*':
                        ...
                        for p in catWord[key]:
                            fsa2.write('('+s+' ('+t+' \"'+p+" "+key+'\" \"'+p+" "+key+'\"))\n')
    fsa2.close()   
